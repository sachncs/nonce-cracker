...
//!
//! Creates mathematically valid ECDSA signature pairs where the nonces are
//! related by `k2 = k1 + delta`.  The generator prints the raw hex values for
//! `r1`, `s1`, `z1`, `r2`, `s2`, `z2`, the public key, and the precomputed
...
//! ```
//!
//! The output can be pasted directly into integration tests or CLI invocations.
...
fn main() {
    let private_key = 0x3039u64;
    let private_scalar = Scalar::from(private_key);

    let delta = 1u64;
    let nonce = 0x1234u64;
    let nonce_scalar = Scalar::from(nonce);
    let next_nonce = nonce + delta;
    let next_nonce_scalar = Scalar::from(next_nonce);

    let r1_point = ProjectivePoint::GENERATOR * nonce_scalar;
    ...
    let r2_point = ProjectivePoint::GENERATOR * next_nonce_scalar;
    ...
    let s1_scalar = (z1_scalar + r1_scalar * private_scalar) * nonce_scalar.invert().unwrap();
    let s2_scalar = (z2_scalar + r2_scalar * private_scalar) * next_nonce_scalar.invert().unwrap();
    ...
    let pk_point = ProjectivePoint::GENERATOR * private_scalar;
    ...
    println!("// Private key d = 0x{private_key:x}");
    println!("// k = 0x{nonce:x}, k' = k + {delta} = 0x{next_nonce:x}");
    println!();
    println!("// Signature 1 (k = 0x{nonce:x}, z1 = 1):");
    ...
    println!();
    println!("// Signature 2 (k' = 0x{next_nonce:x}, z2 = 2):");
    ...
    if computed_d == private_scalar {
        println!("// SUCCESS: d matches expected value 0x{private_key:x}");
    } else {
        println!("// NOTE: computed d != expected d = 0x{private_key:x}");
    }
