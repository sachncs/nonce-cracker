...
impl ProjectivePoint {
    ...
    /// Projective X coordinate (raw, not normalized).
    ///
    /// # Safety
    ///
    /// This is an internal implementation detail exposed for the
    /// `nonce-cracker` projective-coordinate hash optimization.
    /// The value is only meaningful together with `projective_z`.
    #[doc(hidden)]
    pub fn projective_x(&self) -> FieldElement {
        self.x
    }

    /// Projective Z coordinate (raw, not normalized).
    ///
    /// # Safety
    ///
    /// This is an internal implementation detail exposed for the
    /// `nonce-cracker` projective-coordinate hash optimization.
    /// The value is only meaningful together with `projective_x`.
    #[doc(hidden)]
